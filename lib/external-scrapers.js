...
import { fallbackMarketplaceProducts } from "./external-fallback-products.js"
...
const scrapingBeeState = global.scrapingBeeState || { disabledUntil: 0 }
global.scrapingBeeState = scrapingBeeState
...
const MIN_EXTERNAL_PRODUCTS = 5
const MAX_EXTERNAL_PRODUCTS = 8
...
export async function searchExternalPlatforms(filters) {
  ...
  const fallbackProducts = getFallbackMarketplaceProducts(filters, query)

  if (process.env.MARKETPLACE_SEARCH_MODE === "fallback") {
    const fallbackOnly = takeBalancedProducts(fallbackProducts, MAX_EXTERNAL_PRODUCTS)
    externalCache.set(cacheKey, fallbackOnly)
    return fallbackOnly
  }
  ...
  const flattened = buildExternalProductSet(scrapedProducts, fallbackProducts)
...
async function fetchMarketplaceHtml(url, { renderJs }) {
  if (process.env.SCRAPINGBEE_API_KEY && Date.now() > scrapingBeeState.disabledUntil) {
    try {
      ...
      if (/limit|quota|credit|balance|token/i.test(message)) {
        scrapingBeeState.disabledUntil = Date.now() + 12 * 60 * 60 * 1000
      }
      console.warn("ScrapingBee marketplace fetch failed; trying direct request:", message)
    }
  }
...
function buildExternalProductSet(scrapedProducts, fallbackProducts) {
  const scraped = dedupeProducts(scrapedProducts).filter((product) => product.image && product.url)
  const needsBackup = scraped.length < MIN_EXTERNAL_PRODUCTS
  const merged = needsBackup ? dedupeProducts([...scraped, ...fallbackProducts]) : scraped
  return takeBalancedProducts(merged, MAX_EXTERNAL_PRODUCTS)
}

function getFallbackMarketplaceProducts(filters, query) {
  const words = getSearchWords(query)
  const seed = hashString(`${query}|${new Date().toISOString().slice(0, 10)}`)
  const candidates = fallbackMarketplaceProducts
    .filter((product) => priceInRange(product.price, filters.minPrice, filters.maxPrice))
    .map((product) => {
      const meta = platformMeta[product.platform]
      const text = [product.name, product.platform, ...(product.tags || [])].join(" ").toLowerCase()
      const score = words.reduce((total, word) => total + (text.includes(word) ? 1 : 0), 0)
      return {
        score,
        product: normalizeExternalProduct(meta, product),
      }
    })
  const matched = candidates.filter((item) => item.score > 0)
  const source = words.length && matched.length ? matched : candidates
...
function takeBalancedProducts(products, limit) {
  const groups = new Map()
  for (const product of products) {
    const key = product.platform || product.platformName || "marketplace"
    groups.set(key, [...(groups.get(key) || []), product])
  }

  const balanced = []
  while (balanced.length < limit && groups.size) {
...
function getSearchWords(query) {
  return String(query || "")
    .toLowerCase()
    .split(/[^a-z0-9]+/)
    .filter((word) => word.length > 2 && word !== "saree" && word !== "sarees")
}

function seededShuffle(items, seed) {
  const shuffled = [...items]
  let state = parseInt(seed, 36) || 1
